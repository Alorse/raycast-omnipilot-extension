...
export function formatErrorMessage(errorMessage: string): string {
  ...
  if (errorMessage.includes("model not found") || errorMessage.includes("model_not_found")) {
    return `${baseMessage}🔍 **Model not found**\n\n` +
  ...
  if (errorMessage.includes("Invalid API key") || errorMessage.includes("Unauthorized") || errorMessage.includes("401")) {
    return `${baseMessage}🔑 **Invalid API Key**\n\n` +
  ...
  if (errorMessage.includes("Rate limit") || errorMessage.includes("429")) {
    return `${baseMessage}⏱️ **Rate limit exceeded**\n\n` +
  ...
  if (errorMessage.includes("insufficient") || errorMessage.includes("quota") || errorMessage.includes("credits")) {
    return `${baseMessage}💳 **Insufficient funds**\n\n` +
  ...
  if (errorMessage.includes("network") || errorMessage.includes("connection") || errorMessage.includes("fetch")) {
    return `${baseMessage}🌐 **Connection error**\n\n` +
